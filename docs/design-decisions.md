...
- Deploy: the GitHub repo is connected with Root Directory `site`, so every push to `main` deploys to production. For a manual deploy run `vercel deploy --prod` from the **repo root** (the project is linked there; running it inside `site/` fails with "Root Directory site does not exist").
